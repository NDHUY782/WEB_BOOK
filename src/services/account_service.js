const AccountModel = require(`${__path_models}account_model`)
...
module.exports = {
    getAll: async (req) => { // (GetData for LIST, Pagination, Search)
        ...
        let count = await AccountModel.count(condition)
    ...
    countAll: async (req) => { // Filter 
        ...
        let statusFilter = utilsHelpers.createFilterStatus(currentStatus, AccountModel)
        return statusFilter
    },
    ...
    changeStatus: async (req, res) => { // Change status in table
        ...
        AccountModel.updateOne({ _id: id }, data, (err, result) => {
        });
    ...
    deleteItem: async (req, res) => { // Delete one items 
        ...
        await AccountModel.findById(id).then((item) => {
            fileHelpers.remove('src/public/uploads/items/', item.avatar)
        })

        AccountModel.deleteOne({_id:id}, (err,result) => {
    ...
    getForm: async (req) => {  // (GetData for FORM, edit, add)
        ...
        if (id === '') { /// add
            ...
            data = await AccountModel.findById(id)
            pageTitle = 'Edit - Form'
        }
        return {
            pageTitle,
        ...
        uploadAvatar(req, res, async (err) => {
            ...
            if (err) {
                ...
                if (item.id === '') { /// add
                    ...
                    data = await AccountModel.findById(item.id)
                    pageTitle = 'Edit - Form'
                }
                ...
                if (typeof item !== 'undefined' && item.id !== "") { //edit
                    ...
                    AccountModel.updateOne({ _id: item.id }, {
                    ...
                    await new AccountModel(item).save().then(() => {
        ...
        if (action === 'delete') {
            if(Array.isArray(id)){
                for (let index = 0; index < id.length; index++) {
                    await AccountModel.findById(id[index]).then((item) => {
            ...
            AccountModel.deleteMany({ _id: { $in: req.body.cid } }, (err, result) => {
            ...
            AccountModel.updateMany({ _id: { $in: req.body.cid } }, data, (err, result) => {
