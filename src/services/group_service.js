...
module.exports = {
    getAll: async (req) => { // (GetData for LIST, Pagination, Search)
        ...
        let count = await GroupModel.count(condition)
        pagination.totalItem = count

        let data = await GroupModel
            .find(condition)
            .select('name avatar status ordering id_category created modified group_acp')
            .sort(sort)
    ...
    countAll: async (req) => { // Filter 
        ...
        let statusFilter = utilsHelpers.createFilterStatus(currentStatus, GroupModel)
        return statusFilter
    },
    ...
    deleteItem: async (req, res) => { // Delete one items 
        ...
        await GroupModel.findById(id).then((item) => {
    ...
    getForm: async (req) => {  // (GetData for FORM, edit, add)
        ...
        if (id === '') { /// add
            ...
            data = await GroupModel.findById(id)
            pageTitle = 'Edit - Form'
        }
        return {
            pageTitle,
        ...
        uploadAvatar(req, res, async (err) => {
            ...
            if (err) {
                ...
                if (item.id === '') { /// add
                    ...
                    data = await GroupModel.findById(item.id)
                    pageTitle = 'Edit - Form'
                }
                ...
                if (typeof item !== 'undefined' && item.id !== "") { //edit
                    ...
                    await new GroupModel(item).save().then(() => {
        ...
        if (action === 'delete') {
            if(Array.isArray(id)){
                for (let index = 0; index < id.length; index++) {
                    await GroupModel.findById(id[index]).then((item) => {
                        fileHelpers.remove('src/public/uploads/items/', item.avatar)
                    })
                }
            } else {
            ...
            GroupModel.deleteMany({ _id: { $in: req.body.cid } }, (err, result) => {
